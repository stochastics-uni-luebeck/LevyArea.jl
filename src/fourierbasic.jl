...
struct FourierBasic <: AbstractIteratedIntegralAlgorithm end

convorder(::FourierBasic) = 1//2
errcoeff(m, h, ::FourierBasic, ::MaxL2) = h*√3/(√2*π)
norv(m, n, ::FourierBasic) = 2*m*n


"""
    levyarea(W, n, alg::FourierBasic)
...
"""
function levyarea(W::AbstractVector{T}, n::Integer, alg::FourierBasic) where {T<:AbstractFloat}
    m = length(W)
